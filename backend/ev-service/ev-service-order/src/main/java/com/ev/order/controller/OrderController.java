...
public class OrderController {
    ...
    public R<PageResult<OrderVO>> list(@Valid OrderQuery query) { return R.ok(orderService.page(query, null)); }

    @Operation(summary = "订单详情") @GetMapping("/api/orders/{id}")
    public R<OrderVO> detail(@PathVariable Long id) { return R.ok(orderService.detail(id, null)); }
    ...
    public R<PageResult<OrderVO>> userOrders(@Valid UserOrderQuery query) {
        Long userId = SecurityUtils.getUserId();
        return R.ok(orderService.page(query, userId));
    }

    @Operation(summary = "用户端订单详情") @GetMapping("/api/v1/orders/{id}")
    public R<OrderVO> userDetail(@PathVariable Long id) {
        Long userId = SecurityUtils.getUserId();
        return R.ok(orderService.detail(id, userId));
    }
}
