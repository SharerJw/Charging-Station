...
@Slf4j
@Component
public class PayingStateHandler implements OrderStateHandler {

    private final Map<OrderEvent, OrderStatus> eventTransitions = new EnumMap<>(OrderEvent.class);

    public PayingStateHandler() {
        eventTransitions.put(OrderEvent.PAID, OrderStatus.PAID);
    }
    ...
    public void onEnter(ChargingOrderEntity order) {
        log.info("订单进入支付中状态: orderId={}", order.getId());
    }

    @Override
    public void onExit(ChargingOrderEntity order) {
        log.info("订单离开支付中状态: orderId={}, status={}", order.getId(), order.getStatus());
    }
}